...
const cdt2dHelper = {
  computeTriangulation(paths, options, callStack = 0) {
    ...
    let triangles = cdt2d(cdtPoints, cdtEdges, options); // removes degenerated triangles:

    triangles = triangles.filter(triangle => {
      const ba = {
        x: cdtPoints[triangle[1]][0] - cdtPoints[triangle[0]][0],
        y: cdtPoints[triangle[1]][1] - cdtPoints[triangle[0]][1]
      };
      const bc = {
        x: cdtPoints[triangle[1]][0] - cdtPoints[triangle[2]][0],
    ...
    if (paths.length && !triangles.length) {
      let foundProblem = 0; // checks if a point from a path if at the same position than a point from annother path.
        ...
        for (let j = i + 1; j < paths.length; j++) {
          ...
          for (let k = 0; k < path0.length; k++) {
            ...
            for (let l = 0; l < path1.length; l++) {
              ...
              const repulsion = {
                X: Math.max(10, (point1.X - point0.X) / 2),
                Y: Math.max(10, (point1.Y - point0.Y) / 2, 10)
              };
              path0[k] = {
                X: point0.X + repulsion.X,
                Y: point0.Y + repulsion.Y
              };
              path1[l] = {
                X: point1.X - repulsion.X,
                Y: point1.Y - repulsion.Y
              };
              foundProblem++;
            }
          }
      ...
      for (let i = 0; i < paths.length; i++) {
        ...
        for (let j = 0; j < path.length - 1; j++) {
          ...
          for (let k = 0; k < path.length; k++) {
            ...
            const point = path[k];
            if (!cdt2dHelper.isPointOnSegment(point, segmentA, segmentB)) continue; // displace the point:

            const norm = pathHelper.getDistance(segmentA, segmentB);
            const repulsion = {
              X: -(segmentB.Y - segmentA.Y) / norm,
              Y: -(segmentB.X - segmentA.X) / norm
            };
            const vecPath = {
              X: path[(k + 1) % path.length].X - point.X,
              Y: path[(k + 1) % path.length].Y - point.Y
            };
            ...
            path[k] = {
              X: point.X + repulsion.X,
              Y: point.Y + repulsion.Y
            };
            foundProblem++;
          }
        }
  ...
  isPointOnSegment(p, ss, se, threshold = 10) {
    ...
    v.X = v.X / l;
    v.Y = v.Y / l; // the minimal distance from the segment to the point
