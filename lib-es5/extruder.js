...
const extruder = {
  ...
  getGeometry(outerShape, holes, options = {}) {
    ...
    extruder.generateDebugLink(outerShape, holes, options); // get the topology 2D paths by depth
  ...
  getVerticalGeom(innerPathsByDepth, offset = 0, invertNormal = false, mergeMeshes = true) {
    ...
    for (let indexDepth = innerPathsByDepth.length - 1; indexDepth > 0; indexDepth--) {
      const pathsAtDepth = innerPathsByDepth[indexDepth].paths; // for each point at each path at each depth we look for the corresponding point into the upper paths:
      ...
      for (let i = 0; i < pathsAtDepth.length; i++) {
        ...
        for (let idxPtDwn = 0; idxPtDwn < path.length; idxPtDwn++) {
          ...
          const currgeom = geomHelper.getOneVerticalGeom(idxPtDwn, idxNPtDwn, +indexDepth, path, innerPathsByDepth, 0, invertNormal);
  ...
  getDataByDepth(outerShape, holes) {
    ...
    depths.forEach((depth, i) => {
      ...
      const diff = pathHelper.getDiffOfPaths([outerShape.path], keep, {
        polyTree: false
      });
      ...
      outerPaths.push({
        paths: pathHelper.cleanPaths(outerPathAtDepth, 20),
        depth
      });
      innerPaths.push({
        paths: pathHelper.cleanPaths(innerPathAtDepth, 20),
        depth
      });
      let horr;
      ...
      if (i === 0) {
        ...
        if (stop.length > 0) {
          horr = pathHelper.getInterOfPaths(horr, stop, {
            pathPreProcess: false
          });
        }

        horr = pathHelper.getDiffOfPaths(horr, keep, {
          pathPreProcess: false
        });
  ...
  generateDebugLink(outerShape, holes, options) {
    if (options.debug) {
      try {
        ...
        const data64 = pako.deflate(JSON.stringify({
          holes,
          outerShape,
          doNotBuild: options.doNotBuild
        }));
