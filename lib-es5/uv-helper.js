...
const uvHelper = {
  ...
  ***************************************/
  ...
  mapVertical(pathsByDepth, outerShape, options) {
    ...
    const boundaries = uvHelper.getBoundaries(pathsByDepth, outerShape, options); // for each depth: constructs the uvs:
  ...
  getUVertPath(indPath, pathsByDepth, indexDepth, boundaries) {
    ...
    if (path.length === 0) return; // finds into the upper paths if there is a matching edge
