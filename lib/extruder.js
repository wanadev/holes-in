...
const extruder = {
    ...
    getDataByDepth(outerShape, holes) {
        ...
        for (let i = 0; i < holesByDepth.length; i++) {
            ...
            const outerHorizontal = pathHelper.getDiffOfPaths(holesByDepth[i].outer, stackOuter);
            stackOuter = pathHelper.getUnionOfPaths(stackOuter, outerHorizontal);

            horr = pathHelper.getDiffOfPaths(horr,holesByDepth[i].keep);
            horr = pathHelper.cleanPaths(horr, 20);
            horr.push(...outerHorizontal);
