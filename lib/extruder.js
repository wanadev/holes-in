...
const extruder = {
    ...
    getGeometry(outerShape, holes, options = {}) {
        ...
        if (options.frontMesh) {
            let invertFrontNormal = true;
            if (options.doNotInvertFrontNormal) invertFrontNormal = false;
            res.frontMesh = extruder.getHorizontalGeom(horizontalPathsByDepth,innerPathsByDepth, [0], 0, invertFrontNormal);
            uvHelper.mapHorizontal(innerPathsByDepth, outerShape, res.frontMesh, options);
        }
        if (options.backMesh) {
            let invertBackNormal = false;
            if (options.invertBackNormal) invertBackNormal = true;
            res.backMesh = extruder.getHorizontalGeom(horizontalPathsByDepth, innerPathsByDepth, [horizontalPathsByDepth.length - 1], 0, invertBackNormal);
