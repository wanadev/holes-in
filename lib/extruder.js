...
const extruder = {
    ...
    getHorizontalGeom(horizontalPathsByDepth, indexes, offset = 0, invertNormal = true) {
        ...
        for (let i = 0; i < indexes.length; i++) {
            ...
            const allTriangles = horizontalPathsByDepth[indexes[i]].paths.map(path => cdt2dHelper.computeTriangulation([path]));
            const allGeoms = allTriangles.map(triangles =>{
                triangles.depth = horizontalPathsByDepth[indexes[i]].depth;
                return geomHelper.getHorizontalGeom(triangles, 0, invertNormal);
            });
            horrGeom.push(...allGeoms);
        }
